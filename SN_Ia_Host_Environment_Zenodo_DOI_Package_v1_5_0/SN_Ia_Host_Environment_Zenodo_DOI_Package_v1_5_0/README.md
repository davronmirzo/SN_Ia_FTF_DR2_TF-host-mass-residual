...
Pantheon+ and DES-SN5YR comparison products, frozen machine-readable results,
statistical audit code, and final figures in PDF format.
...
influence, and environmental analyses. The broader ZTF layer feeds only the
selection-forward simulations and positive controls. External products feed the
reduced external-product audit.

## Package layout

- `code/` — audit and figure-generation code, including Python and MATLAB
  sources for Figure 1.
- `results/tables/` — frozen machine-readable results and stochastic draws.
...
## Reserved DOI workflow

The archive intentionally uses `10.5281/zenodo.TBD` before a Zenodo draft DOI is reserved.
After reservation, run:

```bash
python checks/set_reserved_doi.py 10.5281/zenodo.XXXXXXX
```

The helper updates every DOI-bearing metadata file, rebuilds the SHA-256
manifest, and reruns verification.
